    ...
    <table>
      ...
      <g:each in="${userInstanceList}" status="i" var="userInstance">
        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
          ...
          <td><g:link class="show" action="show" id="${userInstance?.id}">${fieldValue(bean: userInstance, field: "username")}</g:link></td>

        <td>${fieldValue(bean: userInstance, field: "fullname")}</td>

        <td>${fieldValue(bean: userInstance, field: "specialties")}</td>
