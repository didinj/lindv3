	...
	<h1>Profile Page</h1>
      <div id="featdocheader" class="round6top">
        <table id="featdocheadertable">
          <tr>
            <td width="70%" class="headerrb">${userInstance?.fullname}</td>
            <td width="30%"><g:link controller="message" action="create"><g:img dir="images" file="icon-send-message.png" align="middle" /> Send Message</g:link></td>
          </tr>
        </table> 
      </div>
        ...
        <table>
          <tbody>
            <tr>
              ...
              <td width="40%"><span class="valfont"><g:link controller='search' params='["q":userInstance?.specialties]' action='index'>${userInstance?.specialties}</g:link></span></td>
              <td rowspan="4" class="score center" width="20%"><span class="nilai-score valfont">${fieldValue(bean: result, field: "points")}</span></td>
            </tr>
            <tr>
    ...
    <div id="rightpublicfront">
      ...
      <div id="friendlist">
        ...
        <g:each in="${connInstance}" var="conn"> 
          <g:if test="${conn.id}==${currentUser.id}">
            <table>
              <tr>
                <td rowspan="2"><img class="avatar-small" src="${createLink(controller:'public', action:'avatar_image', id:conn.id)}" /></td>
                <td>${conn.fullname}<br />
${conn.specialties}
                </td>
              </tr>
              <tr>
                <td><g:img dir="images" file="icon-send-message.png" align="middle" /> Send Message</td>
              </tr>
            </table>
          </g:if>
          <g:else>
...
${conn.specialties}
              ...
              <tr>
                <td><g:link controller="message" action="create"><g:img dir="images" file="icon-send-message.png" align="middle" /> Send Message</g:link></td>
              </tr>
            </table>
          </g:else>
        </g:each>  
